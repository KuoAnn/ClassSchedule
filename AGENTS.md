...
**畫面上的窄版是緊密排列，時間軸與逐時對齊只存在於匯出的直式圖片。**
兩邊的用途不一樣，所以刻意分開：
螢幕上是「一天一串」往下滑，空班留白只是逼人多滑；
匯出的是一張拿來比整週的海報，逐時對齊才有價值，也才有東西可以對齊。

一旦某天自己把卡片往上收，七天共用的那一欄時間就對不上任何一張卡
（`10:00`、`11:00` 會指到別的時段），所以**時間欄與對齊是綁在一起的、要收就一起收**：
螢幕上兩者都收（`04-narrow.css` 的 `.ngut{display:none}`、`.hrow.empty{display:none}`，
`syncRows()` 遇到沒有 `npack` 就把 inline 高度清掉直接 return）；
匯出時 `npack` 再把兩者一起裝回來。每張卡自己都印著時段，螢幕上少的只有
早／午／晚的分段圖示。`.hrow` 的 `border-top` 同理 —— 它代表「換一個小時」，
緊密排列後沒有意義。

收掉時間欄還順手多了 48px 的寬度給日欄：390px 的手機從 152px 變成 176px（仍是兩天）。
`.ndockgut`（釘住的星期列讓開時間欄用的墊片）要跟著收，不然整排星期會偏移一個時間欄的寬度。

空堂的留白改從卡片本身省：`08-export.js` 在窄版匯出時加 `npack`（`05-export-mode.css`），
壓掉 `.lc` 的內距、行距、`.bt` 的上緣間距，並把空的 `.bt` 收掉。
螢幕上那些留白是為了好點擊，印成圖只是白費高度。
